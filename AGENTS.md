...
- After rebasing multiple feature branches into `fork/feature-requests/roll-up`, do not stop at focused Vitest slices. We hit a case where focused runtime/web tests passed, but `web-ui` full typecheck and `npm run build` still failed due to merged `displayPath` contract drift and duplicate test harness props in `web-ui/src/hooks/use-board-interactions*`. For roll-up validation, always run at least `cd web-ui && npm run typecheck` and preferably full `npm run build` before calling the branch clean.
